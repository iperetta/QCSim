...
def plot_qubits(*args, **kwargs):
    ...
    def bloch_sphere(ax):
        ...
        ax.plot3D(np.cos(u), np.sin(u), np.zeros(u.shape), color='lightblue', linestyle='-.')
        ax.plot3D(np.cos(u), np.zeros(u.shape), np.sin(u), color='lightblue', linestyle='-.')
        ax.plot3D(np.zeros(u.shape), np.cos(u), np.sin(u), color='lightblue', linestyle='-.')
